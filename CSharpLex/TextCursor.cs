...
namespace LexSharp
{
   internal class TextCursor<T>
   {
      ...
      public ScanResult<T> GetNextToken()
      {
         ...
         var result = new ScanResult<T>() { Token = minimalPatternIndexAndLongestMininimalMatches.Select(x => new Token<T>(x.Match.Value, x.Pattern.TokenType, x.Match.Index, x.Match.Length)).FirstOrDefault(), IsValid = minimalPatternIndexAndLongestMininimalMatches.Count() > 0 };
         if( result.IsValid)
         {
